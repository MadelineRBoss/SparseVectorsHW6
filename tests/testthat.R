...
test_that("show", {
  result <- c("The sparse vector of size 5 has the following values: ",
              "position: 1 , value: 2 ",
              "position: 2 , value: 2 ",
              "position: 3 , value: 2 ",
              "position: 4 , value: 2 ",
...
test_that("check coercion returns numeric", {
  ...
  expect_type(x, "double")     # numeric vectors are "double" in R
  expect_true(is.numeric(x))   # also valid
})
